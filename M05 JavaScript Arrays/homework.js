...
function promedioResultadosTest(resultadosTest) {
   ...
   // Tu código:
   j=0
   for(u=0;u<resultadosTest.length;u++){
      j=j+resultadosTest[u]
   }
   return j/u
}
...
function numeroMasGrande(arrayOfNums) {
   ...
   // Tu código:
   let g=arrayOfNums[0]
   for(let e=0;e<arrayOfNums.length;e++){
      if(g<arrayOfNums[e]){g=arrayOfNums[e]}
   }
   return g
}
...
function multiplicarArgumentos() {
   ...
   for(let r=0;r<arguments.length;r++){
      if(arguments[r]!=undefined){
         p=p*arguments[r]} else{c++}
   }
   if(p===1&&c>0||arguments.length===0){return 0}else{return p}
}
...
function cuentoElementos(array) {
   ...
   // Tu código:
   let m=0
   for(let r=0;r<array.length;r++){if(array[r]>18){m++}}
   return m
}
...
function diaDeLaSemana(numeroDeDia) {
   ...
   // Tu código:
      let p=numeroDeDia
      if(p===1||p===7){return"Es fin de semana"}else{return"Es dia laboral"}
}
